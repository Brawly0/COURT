...
namespace CaseClosed.EditorTools.Match
{
    ...
    public static class MatchPhaseAudit
    {
        ...
        public static string Run(out bool allPassed)
        {
            ...
            // the aisle runs through the CourtroomCentre marker when a map
            // provides one (seating is marker-relative); greybox default is x=0
            var centreGo = GameObject.Find("CourtroomCentre");
            float aisleX = centreGo != null ? centreGo.transform.position.x : 0f;
            Check("opposing counsel are on opposite sides",
                  (CourtroomSeating.SpotFor(PlayerRole.Prosecutor).x - aisleX) *
                  (CourtroomSeating.SpotFor(PlayerRole.DefenseAttorney).x - aisleX) < 0f,
